...
LLM Client Wrapper
------------------
Provides a unified interface for calling any OpenAI-compatible LLM API.
All modules import this to ensure consistent parameter passing.

Telemetry Integration
---------------------
If a ``TelemetryCollector`` is active for the current thread (registered via
``telemetry.set_active_collector()``), every ``call_llm()`` invocation
automatically records its prompt/completion token counts and wall-clock
latency into that collector.  No changes are required at the call sites —
telemetry is purely opt-in at the thread level.
"""
