...
def run_cultural_hypothesis(
    ...
    top_country = max(pruned, key=pruned.get)
    hofstede_scores = HOFSTEDE_DB[top_country].copy()

    if debug:
        print(_section("Inferred Hofstede Scores"))
        print(f"  Selection mode  : {_c('top-country fixed Hofstede vector', _BOLD, _CYAN)}")
        print(f"  Selected country: {_c(top_country, _GREEN, _BOLD)} "
              f"{_c(f'({pruned[top_country]*100:.1f}%)', _DIM)}")
        print(f"  Fixed-vector    : {_hofstede_row(hofstede_scores)}")
        for dim in DIMENSIONS:
    ...
    cultural_state = {
        ...
        "selection_mode": "top_country_fixed_hofstede",
