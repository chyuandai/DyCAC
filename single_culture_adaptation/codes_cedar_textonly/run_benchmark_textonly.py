...
class Evaluator:
    ...
    def __init__(self, data_dir: str, model: ModelInterface, output_dir: str = "results", max_workers: int = 8):
    ...
    def _evaluate_item(self, language: str, idx: int, item_data: Dict, template: str) -> Tuple[int, EvaluationItem]:
        """Evaluate one benchmark item. Designed to be safe for ThreadPoolExecutor."""
        try:
    ...
    def _process_language(self, language: str, data_list: List[Dict]):
        ...
        workers = min(self.max_workers, len(items_to_process))
        logger.info(f"process {language} with {workers} concurrent workers")

        completed: List[Tuple[int, EvaluationItem]] = []
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(self._evaluate_item, language, idx, item_data, template): idx
                for idx, item_data in items_to_process
            }
            with tqdm(total=len(futures), desc=f"process {language.ljust(8)}", unit="item") as pbar:
                for future in as_completed(futures):
                    idx, eval_item = future.result()
                    completed.append((idx, eval_item))
                    self._save_checkpoint(language, idx)
                    pbar.update(1)

        for _, eval_item in sorted(completed, key=lambda x: x[0]):
