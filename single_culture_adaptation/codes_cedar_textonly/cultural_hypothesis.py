...
def run_cultural_hypothesis(
    ...
    """
    Infer Hofstede cultural dimension scores using a top-1 country ablation.
    ...
    if not hypotheses:
        if debug:
            print(f"\n  {_c('No valid hypotheses - fallback to global default scores.', _RED)}")
        return GLOBAL_DEFAULT.copy(), {"hypotheses": {}, "scores": GLOBAL_DEFAULT.copy()}

    selected_country = max(hypotheses, key=hypotheses.get)
    selected_prob = hypotheses[selected_country]
    hofstede_scores = HOFSTEDE_DB[selected_country].copy()

    if debug:
        print(_section("Top-1 Country Distribution"))
        print(_prob_bars(hypotheses))
        print(f"\n  {_c('Selected country:', _BOLD)} {_c(selected_country, _GREEN, _BOLD)} "
              f"{_c(f'({selected_prob*100:.1f}%)', _DIM)}")
        print(_section("Fixed Hofstede Scores"))
        print(f"  Selected-fixed : {_hofstede_row(hofstede_scores)}")
        for dim in DIMENSIONS:
