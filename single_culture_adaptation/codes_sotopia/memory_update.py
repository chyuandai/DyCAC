...
# ---------------------------------------------------------------------------
# Sub-prompt T — Theory-of-Mind Inference
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Sub-prompt M — Memory Operations
# ---------------------------------------------------------------------------
...
COMBINED_MEMORY_SYSTEM_PROMPT = _TOM_SYSTEM  # kept for import compatibility
...
def _extract_json(raw: str):
    ...
    # Step 4 — greedy search for outermost JSON object or array
    for pattern in (r"\{.*\}", r"\[.*\]"):
...
# ---------------------------------------------------------------------------
# Instruction executor  (unchanged from original)
# ---------------------------------------------------------------------------
...
def _execute_instructions(
    ...
    # Auto-correction map: perception field names the model sometimes uses by mistake
    # → the correct BDI category they should be stored under.
    _BDI_KEY_COERCE: dict[str, str] = {
        "values_and_obsessions": BDI_BELIEFS,
        "immediate_intent":      BDI_INTENTIONS,
        "emotion":               BDI_BELIEFS,
        "communication_style":   None,   # belongs to world_facts; handled below
    ...
    for idx, op in enumerate(instructions):
        ...
        if layer == LAYER_WORLD:
            ...
            # Auto-correct: if the model used a perception field name as bdi_key,
            # remap it to the appropriate BDI category instead of discarding.
            if bdi_key not in (BDI_BELIEFS, BDI_DESIRES, BDI_INTENTIONS):
                ...
                if coerced is None and bdi_key in _BDI_KEY_COERCE:
                    # Perception-style field that belongs in world_facts, not mental_state
                    layer = LAYER_WORLD
                    bdi_key = None
                    target = memory[LAYER_WORLD]
                    logger.debug(
...
# ---------------------------------------------------------------------------
# Dialogue-meta updater  (unchanged)
# ---------------------------------------------------------------------------
...
def run_memory_update(
    ...
    # threading.local() is NOT inherited by child threads. Capture the active
    # collector here and re-register it inside each sub-thread so call_llm()
    # can record its token counts into the parent turn's TelemetryCollector.
    try:
    ...
    # Shared user-message body for both sub-calls
    ...
    # ── Sub-call helpers with retry ───────────────────────────────────────

    def _call_tom() -> dict:
        if _parent_collector is not None:
            set_active_collector(_parent_collector)
        try:
          user_msg = _TOM_USER_TMPL.format(
            memory_json=memory_json,
            perception_json=perception_json,
          )
          last_err = None
          for attempt in range(1, max_retries + 1):
            raw = call_llm(system_prompt=_TOM_SYSTEM, user_message=user_msg,
                           _module="memory_tom", **llm_kw)
            try:
                ...
                last_err = e
                if debug:
                    print(f"  {_c(f'⚠ [ToM] attempt {attempt}: {e}', _RED)}")
          if debug:
            print(f"  {_c('⚠ [ToM] all retries exhausted — fallback to empty', _RED)}")
          return {}
        finally:
            if _parent_collector is not None:
                _tel_clear()
    ...
    # ── Inject hidden desires from ToM ────────────────────────────────────
