...
Multi-turn interactive mode (omit --input):
    python main.py --api_key sk-... --model_name gpt-4o --debug
"""
...
def ingest_input(
    ...
    """
    # NOTE: Data parsing / format normalization logic should be inserted here.
    # Currently returns inputs as-is (interface stub).
...
def run_pipeline(
    ...
    n_hypotheses: int,
    debug: bool,
    # Social goal (SOTOPIA-inspired goal-directed mode)
    social_goal: str = "",
    ...
    # ── Module 2 & 3: Memory Update + Cultural Hypothesis (parallel) ─────
    # Both modules depend only on `perception` and are independent of each
    # other, so they can safely run concurrently to reduce wall-clock time.
    def _run_memory():
    ...
    def _run_culture():
        return run_cultural_hypothesis(
            perception=perception,
            current_input=analysis_input,
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        future_memory  = executor.submit(_run_memory)
        future_culture = executor.submit(_run_culture)
        updated_memory                    = future_memory.result()
        hofstede_scores, updated_cultural_state = future_culture.result()
...
def main():
    ...
    if turns is not None:
        # Batch mode: process each turn sequentially
        for t, turn_input in enumerate(turns):
            ...
            if args.debug:
                # Compact cultural state summary
