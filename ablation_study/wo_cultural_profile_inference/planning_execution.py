...
MEMORY_CONTEXT_TEMPLATE = """You are a context-aware conversational assistant.
...
Response style guidance (this affects HOW you pursue the goal, not WHETHER):
{style_guidelines}
...
def _build_style_guidelines(memory: dict, goal_tracking: dict = None, max_guidelines: int = 6) -> str:
    """Build response-style guidance without Cultural Hypothesis or culture-dimension scores."""
    wf = memory.get("world_facts", {}) or {}
    mental = memory.get("mental_state", {}) or {}
    gt = goal_tracking or {}
    guidelines = []

    relationship = str(wf.get("relationship", "") or "").strip()
    if relationship:
        guidelines.append(f"- Preserve the established relationship context: {relationship}.")

    beliefs = mental.get("beliefs", {}) or {}
    salient_belief = ""
    for _, entry in sorted(beliefs.items(), key=lambda kv: _safe_entry_conf(kv[1]), reverse=True)[:2]:
        value = _safe_entry_value(entry)
        if value:
            salient_belief = value
            break
    if salient_belief:
        guidelines.append(f"- Acknowledge salient inferred concern without over-explaining: {salient_belief}.")

    stance = str(gt.get("partner_stance", "") or "").strip()
    if stance:
        guidelines.append(f"- Adapt to the counterpart stance detected by Perception: {stance}.")

    obstacles = _compact_list(gt.get("obstacles", []) or [], limit=2)
    if obstacles:
        guidelines.append(f"- Address the main blocker directly: {'; '.join(obstacles)}.")

    leverage = _compact_list(gt.get("leverage_points", []) or [], limit=2)
    if leverage:
        guidelines.append(f"- Use concrete leverage or openings already present: {'; '.join(leverage)}.")

    guidelines.append("- Be concise, specific, and grounded in the latest turn.")
    guidelines.append("- Avoid stereotyping, country inference, or culture-dimension assumptions.")
    return "\n".join(guidelines[:max_guidelines])
...
def _derive_strategy(memory: dict, social_goal: str, goal_tracking: dict) -> str:
    ...
    lines.append("- prefer ask -> align -> propose over generic empathy or generic mirroring")
...
def run_planning_execution(
    ...
    style_guidelines = _build_style_guidelines(memory, goal_tracking or {})
    ...
    if debug:
        ...
        print(_section("Style Guidance"))
        for line in style_guidelines.splitlines():
